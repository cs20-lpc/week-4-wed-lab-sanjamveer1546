...
void DoublyList<T>::append(const T &elem)
{
    ...
    Node *n = new Node(elem, trailer, before);
...
T DoublyList<T>::getElement(int position) const
{
    ...
    {
        throw out_of_range("getElement: position out of range");
    }
...
void DoublyList<T>::remove(int position)
{
    if (position < 0 || position >= this->length)
    {
        throw out_of_range("remove: position out of range");
    }
...
void DoublyList<T>::replace(int position, const T &elem)
{
    if (position < 0 || position >= this->length)
    {
        throw out_of_range("replace: position out of range");
    }
